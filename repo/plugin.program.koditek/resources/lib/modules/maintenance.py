...
from .addonvar import (
    currSkin, user_path, db_path, addon_name, textures_db, advancedsettings_folder,
    advancedsettings_xml, dialog, dp, xbmcPath, packages, setting_set, addon_icon,
    local_string, addons_db
)
...
def advanced_settings():
    selection = xbmcgui.Dialog().select(
        local_string(30038),
        [
            '1GB Devices (E.g. 1st-3rd gen Firestick/Firestick Lite)',
            '1.5GB Devices (E.g. 4k Firestick)',
            '2GB+ Devices (E.g. Shield Pro/Tube, FireTV Cube, Apple Tv 4k)',
            'Default (Reset to Default)',
            local_string(30039)
        ]
    )  # Select Ram Size, Delete
...
def fresh_start(standalone=False):
    if standalone:
        ...
        yesFresh = dialog.yesno(
            local_string(30012), local_string(30042),
